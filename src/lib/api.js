...
export const api = {
  ...
  confirmReplacement:     (d)      => call('confirmReplacement', d),
  ...
  clearRecords:           (d)      => call('clearRecords', d),
}
...
const MOCK = (() => {
  ...
  // Minimal seed — no mock classes, areas reflect real setup
  const DEFAULT_DB = {
    students: [
      { id: 'S001', admNo: 'ADM001', name: 'Arjun Nair',    class: '10-A', status: 'Active',   completedCount: 12, manualCredit: 0, pendingCount: 1, lastDutyDate: '2026-05-10' },
      { id: 'S002', admNo: 'ADM002', name: 'Priya Menon',   class: '10-B', status: 'Active',   completedCount: 8,  manualCredit: 0, pendingCount: 2, lastDutyDate: '2026-05-12' },
      { id: 'S003', admNo: 'ADM003', name: 'Rahul Krishna', class: '11-A', status: 'Leave',    completedCount: 15, manualCredit: 0, pendingCount: 0, lastDutyDate: '2026-05-05' },
      { id: 'S004', admNo: 'ADM004', name: 'Sneha Das',     class: '9-C',  status: 'Active',   completedCount: 5,  manualCredit: 0, pendingCount: 2, lastDutyDate: '2026-05-14' },
      { id: 'S005', admNo: 'ADM005', name: 'Aditya Sharma', class: '12-A', status: 'Active',   completedCount: 20, manualCredit: 0, pendingCount: 0, lastDutyDate: '2026-05-16' },
      { id: 'S006', admNo: 'ADM006', name: 'Meera Pillai',  class: '8-B',  status: 'Active',   completedCount: 4,  manualCredit: 0, pendingCount: 1, lastDutyDate: '2026-05-13' },
      { id: 'S007', admNo: 'ADM007', name: 'Vikram Patel',  class: '11-B', status: 'Inactive', completedCount: 0,  manualCredit: 0, pendingCount: 0, lastDutyDate: null },
      { id: 'S008', admNo: 'ADM008', name: 'Anjali Reddy',  class: '9-A',  status: 'Active',   completedCount: 9,  manualCredit: 0, pendingCount: 0, lastDutyDate: '2026-05-15' },
    ],
    areas: [
      { id: 'A001', name: 'Main Corridor',     type: 'Daily',     requiredCount: 4, eligibleClasses: [], active: true },
      { id: 'A002', name: 'Dining Hall',       type: 'Daily',     requiredCount: 6, eligibleClasses: [], active: true },
      { id: 'A003', name: 'Outdoor Courtyard', type: 'Daily',     requiredCount: 3, eligibleClasses: [], active: true },
      { id: 'A004', name: 'Washrooms Block A', type: 'Daily',     requiredCount: 2, eligibleClasses: [], active: true },
      { id: 'A005', name: 'Library Hall',      type: 'Daily',     requiredCount: 2, eligibleClasses: [], active: true },
      { id: 'A006', name: 'Event Hall',        type: 'Non-Daily', requiredCount: 8, eligibleClasses: [], active: true },
      { id: 'A007', name: 'Sports Ground',     type: 'Non-Daily', requiredCount: 5, eligibleClasses: [], active: true },
    ],
    duties: [
      { id: 'D001', date: TODAY(), areaId: 'A001', areaName: 'Main Corridor',     studentId: 'S001', studentName: 'Arjun Nair',   class: '10-A', type: 'Daily', status: 'Pending',  generatedAt: TODAY() },
      { id: 'D002', date: TODAY(), areaId: 'A002', areaName: 'Dining Hall',       studentId: 'S002', studentName: 'Priya Menon',  class: '10-B', type: 'Daily', status: 'Done',     generatedAt: TODAY() },
      { id: 'D003', date: TODAY(), areaId: 'A003', areaName: 'Outdoor Courtyard', studentId: 'S004', studentName: 'Sneha Das',    class: '9-C',  type: 'Daily', status: 'Not Done', generatedAt: TODAY() },
      { id: 'D004', date: TODAY(), areaId: 'A004', areaName: 'Washrooms Block A', studentId: 'S008', studentName: 'Anjali Reddy', class: '9-A',  type: 'Daily', status: 'Pending',  generatedAt: TODAY() },
    ],
    credits: [],
  }
        ...
        switch (action) {
          ...
          case 'getStudents': {
            ...
            if (data.search) {
              ...
              list = list.filter(s => s.name.toLowerCase().includes(q) || s.admNo.toLowerCase().includes(q) || s.class.toLowerCase().includes(q))
            }
            return list.map(s => ({ ...s, effectiveCount: eff(s) }))
          }
          case 'saveStudent': {
            // admNo IS the studentId
            if (db.students.find(s => s.admNo === data.admNo)) throw new Error('Admission number already exists')
            const s = { ...data, id: data.admNo, completedCount: 0, manualCredit: 0, pendingCount: 0, lastDutyDate: null }
          ...
          case 'cancelDuty': {
            const d = db.duties.find(d => d.id === data.assignmentId)
            if (d) {
              const wasP = d.status === 'Pending'
              d.status = 'Cancelled'
              if (wasP) { const s = db.students.find(s => s.id === d.studentId); if (s) s.pendingCount = Math.max(0, s.pendingCount - 1) }
            }
            save(db); return true
          }
          ...
          case 'confirmReplacement': {
            // Swap student on the duty
            const { assignmentId, newStudentId } = data
            const d = db.duties.find(d => d.id === assignmentId)
            const newS = db.students.find(s => s.id === newStudentId)
            const oldS = db.students.find(s => s.id === d?.studentId)
            if (!d || !newS) return false
            // Restore old student pending count
            if (oldS && d.status === 'Pending') oldS.pendingCount = Math.max(0, oldS.pendingCount - 1)
            d.studentId   = newS.id
            d.studentName = newS.name
            d.class       = newS.class
            if (d.status === 'Pending') newS.pendingCount++
            save(db); return true
          }
          ...
          case 'generateDailyDuties': {
            ...
            const generated = []; const usedPerDay = {}
            for (let i = 0; i < days; i++) {
              ...
              for (const area of areas) {
                const pool = active.filter(s =>
                  !usedPerDay[ds].has(s.id) &&
                  (area.eligibleClasses.length === 0 || area.eligibleClasses.some(c => s.class.startsWith(c)))
          ...
          case 'getDashboard': {
            ...
            const completed   = todayDuties.filter(d => d.status === 'Done').length
            const total       = todayDuties.length
            return {
